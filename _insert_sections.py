...
\t\t\t\t\t\t\t\t\t\t\t<p>Supported instruction for remote sensing and GIS courses with combined enrollment of 98+ students across four course sections. Developed lab exercises, graded assignments, and delivered guest lectures.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Undergraduate introductory course covering map projections, coordinate systems, aerial photography, and satellite image interpretation. Emphasis on practical lab skills.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Graduate-level course on electromagnetic spectrum theory, multi/hyperspectral sensor systems, image classification, and environmental applications including ocean color, land cover, and vegetation analysis.</p>
...
\t\t\t\t\t\t\t\t\t\t\t<p>Original syllabi, course materials, and instructional resources developed for prospective or current teaching. Each course reflects research-integrated pedagogy with applied data science and field components.</p>
\t\t\t\t\t\t\t\t\t\t\t<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:0.8rem;margin-top:0.8rem;">
\t\t\t\t\t\t\t\t\t\t\t\t<div style="background:var(--bg-card);border:1px solid rgba(0,206,209,0.15);border-radius:8px;padding:0.85rem 1rem;">
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Survey course covering ocean basins, physical &amp; chemical oceanography, biological productivity, and human impacts on the ocean. Designed for undergraduate non-majors.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Foundational GIS course covering vector/raster data models, coordinate reference systems, spatial analysis, and QGIS/ArcGIS applications for environmental problem-solving.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Advanced course on marine remote sensing: ocean color algorithms, SAR for sea ice and oil spills, habitat mapping, and Google Earth Engine workflows for coastal monitoring.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Advanced course on deep learning for satellite imagery: CNNs, U-Net segmentation, transformer-based models, and foundation models (Prithvi, DINO) for geospatial applications.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Hands-on course covering classical ML (regression, classification, clustering), model evaluation, and scikit-learn applications to oceanographic and environmental datasets.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Integrates GIS, Python, and statistical analysis for solving spatial environmental problems; covers data wrangling, geostatistics, spatial regression, and interactive dashboards.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Covers distributed computing, GEE at scale, AWS/cloud storage, large-scale raster analytics, and parallel Python (Dask, Ray) for petabyte-scale geospatial datasets.</p>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t<p style="font-size:0.84rem;color:#8aabb0;margin:0.3rem 0 0.5rem;">Graduate course on analysis of observational oceanographic and atmospheric data: time series, spectral analysis, EOF/PCA, empirical mode decomposition, and reanalysis products.</p>
...
\t\t\t\t\t\t\t\t\t\t<p>Machine learning and satellite remote sensing for chlorophyll-a, turbidity, CDOM, and dissolved oxygen estimation. Hypoxia dynamics in the northern Gulf of Mexico and Mississippi Sound driven by nutrient loading and stratification.</p>
...
\t\t\t\t\t\t\t\t\t\t<p>Ensemble ML, deep learning (CNN, LSTM, Transformers), and foundation model fine-tuning for oceanographic prediction tasks: water quality retrieval, plankton classification, habitat mapping, and eddy detection.</p>
...
# Find the exact insertion point: just before "<!-- Section --> <section id="about">"
# We look for the pattern of publications section closing + Section comment + about section opening
old_marker = '								</section>\n\n\t\t\t\t\t\t<!-- Section -->\n\t\t\t\t\t\t\t\t<section id="about">'

if old_marker in content:
    new_content = content.replace(old_marker, '								</section>\n' + new_sections + '\n\t\t\t\t\t\t<!-- Section -->\n\t\t\t\t\t\t\t\t<section id="about">', 1)
    with open(path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print(f'SUCCESS. New file size: {len(new_content):,} chars')
else:
    # Try alternative whitespace
    import re
    # Find the closing of publications section
    m = re.search(r'\t+</section>\s*\n\s*<!--\s*Section\s*-->\s*\n\s*<section id="about">', content)
    if m:
        print(f'Found at pos {m.start()}: {repr(content[m.start():m.start()+120])}')
    else:
        # Show the area around "about" section
        idx = content.find('<section id="about">')
        print(f'About section found at index {idx}')
